...
ECU_Name = "IPC"
Network = "CANFD Network"
Channel = 1
Baudrate = 500000

print("Connected with Channel 1")

BCM_info = {"ID": hex(0x110), "Tx Method": "Cyclic", "Cycle Time": 250, "Channel": 1, "DLC": 1, "Data":{"HMI_Warn_Sig":1 }}
...
def TransmitMsg():
    try:
            ...
            channel.exchange_declare(exchange= 'IPCMessage', exchange_type=ExchangeType.fanout)
            while True:
                result=json.dumps(BCM_info)
                print("Transmitting..", result)

                channel.basic_publish(exchange = 'IPCMessage', routing_key = "", body = result)
...
def onmsg():
    ...
    channel.queue_bind(exchange="BCMMessage",queue=queue.method.queue)
    ...
    print("waiting for message . To xit Press CTRL+C")
